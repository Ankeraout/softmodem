...
SYMBOL_FREQUENCIES = {
    modem.DataDirection.RX: {
        modem.Role.CALLER: [1850, 1650],
        modem.Role.CALLEE: [1180, 980],
    },
    modem.DataDirection.TX: {
        modem.Role.CALLER: [1180, 980],
        modem.Role.CALLEE: [1850, 1650]
    }
}

class V21Receiver(modem.analog_protocols.fsk.FSKReceiver):
...
class V21Sender(modem.analog_protocols.fsk.FSKSender):
...
class V21(modem.IAnalogProtocol):
    def __init__(
        self,
        ...
        self._analog_receiver = V21Receiver(bit_protocol, role)
        self._analog_sender = V21Sender(bit_protocol, role)
        self._modem = modem
