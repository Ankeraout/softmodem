...
class V250(modem.IByteProtocol):
    def __init__(self, phone: modem.IPhone, _modem: modem.modem.Modem) -> None:
        self._phone: modem.IPhone = phone
        self._modem = _modem
        self._byte_receiver: modem.IByteReceiver | None = None
    ...
    @property
    def byte_receiver(self) -> modem.IByteReceiver | None:
        return self._byte_receiver
    
    @byte_receiver.setter
    def byte_receiver(self, byte_receiver: modem.IByteReceiver | None) -> None:
    ...
    def _send_byte(self, byte: int) -> None:
        with self._modem._lock:
            self._modem.bit_protocol.send_bytes(bytes([byte]))
        ...
        while True:
            with self._modem._lock:
                if self._state in (
                    ...
                    if self._modem.call_state == modem.CallState.ENDED:
