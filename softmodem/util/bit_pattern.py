...
class PatternBitProvider(softmodem.IBitProvider):
...
class PatternBitReceiver(softmodem.IBitReceiver):
    def __init__(self, pattern: list[int]) -> None:
        self._pattern = pattern
        self._index = 0
        self._count = 0
...
class TransitionCounter(softmodem.IBitReceiver):
...
class BitReceiverSplitter(softmodem.IBitReceiver):
    def __init__(self, receivers: list[softmodem.IBitReceiver]) -> None:
