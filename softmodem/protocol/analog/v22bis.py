import cmath
import enum
import softmodem.util.agc
import softmodem.util.bit_pattern
import softmodem.util.costas
import softmodem.util.muller
import softmodem.util.goertzel
import softmodem.util.rrc
import softmodem.util.scramble
import softmodem.util.slice
import scipy.signal
import typing
...
class V22bisReceiver(softmodem.IAnalogReceiver):
    def __init__(
        self,
        ...
        self._filter = softmodem.util.rrc.rrc(
            0.75,
        ...
        self._phase_advance *= (
            2400 if call_direction == softmodem.CallDirection.OUTGOING
            else 1200
        )
        self._muller = softmodem.util.muller.Muller(
            INTERNAL_SAMPLES_PER_SYMBOL,
            softmodem.util.slice.slicer_qam16,
            0.1,
            0.00001
        )
        ...
        self._unscrambler = softmodem.util.scramble.Unscrambler(
            0x24001,
            0x18475
        )
        self.enable_transition_counter: bool = True
        self._transition_counter = softmodem.util.bit_pattern.TransitionCounter(
            int(2400 * 0.27)
        )
        self._costas = softmodem.util.costas.Costas(
            softmodem.util.slice.slicer_qam16,
            0.1,
            0.0001,
            40
        )
        self._agc = softmodem.util.agc.AGC(0.0001)
        self.speed = 2400
    ...
    def speed(self, value: int) -> None:
        ...
        slicer = (
            softmodem.util.slice.slicer_qam16 if value == 2400
            else softmodem.util.slice.slicer_qpsk_v22bis
        )
    ...
    def receive_samples(self, samples: list[float]) -> None:
        ...
        for symbol in self._muller.receive_samples(samples_fixed):
...
class V22bisSender(softmodem.IAnalogProvider):
    def __init__(
        self,
        call_direction: softmodem.CallDirection,
        ...
        self._phase_advance *= (
            1200 if call_direction == softmodem.CallDirection.OUTGOING
            else 2400
        )
        self._quadrant_phase = 0
        self._scrambler = softmodem.util.scramble.Scrambler(0x24001, 0x18475)
...
class V22bis(softmodem.IAnalogProtocol):
    ...
    def __init__(
        ...
        self._tone_detector = softmodem.util.goertzel.SlidingGoertzel(
            SAMPLE_RATE_EXTERNAL,
            (
                2250 if call_direction == softmodem.CallDirection.OUTGOING
                else 1050
            ),
            800
        )
        ...
        self._pattern_provider_0011 = softmodem.util.bit_pattern.PatternBitProvider(
            [0, 0, 1, 1]
        )
        self._pattern_provider_1 = softmodem.util.bit_pattern.PatternBitProvider(
            [1]
        )
        self._handshake_bit_receiver = softmodem.util.bit_pattern.BitReceiverSplitter(
            [
                self._pattern_receiver_0011,
                self._pattern_receiver_1
            ]
        )
        self._receiver = V22bisReceiver(
            call_direction,
            self._handshake_bit_receiver
        )
        self._sender = V22bisSender(
            call_direction,
            self._pattern_provider_1
        )
